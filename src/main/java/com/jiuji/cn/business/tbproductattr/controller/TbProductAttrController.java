package com.jiuji.cn.business.tbproductattr.controller; 
...
/** 
 *  TbProductAttrController
 * 
 */
@Controller
@RequestMapping(value="/tbproductattr/tbProductAttrController")
public class TbProductAttrController extends BaseController {
	
	String menuUrl = "tbproductattr/tbProductAttrController/list.do";  
	...
	public ResultAction save() throws Exception{
		logBefore(logger, "TbProductAttrController");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();     
		int F_PRODUCT_ATTR_UID = tbProductAttrService.save(pd);
		...
		if(!"".equals(PICTURES_IDs)){
			for(int i=0;i<PICTURES_IDs.length;i++){
				...
				picture.put("TABLE_NAME", "TB_PRODUCT_ATTR");			 
				picture.put("TABLE_UID_VALUE",F_PRODUCT_ATTR_UID);						  
	...
	public ResultAction delete(){
		...
		try{
			pd = this.getPageData();
			tbProductAttrService.delete(pd); 
		} catch(Exception e){
	...
	public ResultAction edit(HttpServletRequest request) throws Exception{
		...
		if(!"".equals(PICTURES_IDs)&&PICTURES_IDs!=null){
			for(int i=0;i<PICTURES_IDs.length;i++){
				...
				PageData picture = picturesService.findById(pd); 
				picture.put("TABLE_NAME", "TB_PRODUCT_ATTR");			 
				picture.put("TABLE_UID_VALUE",pd.getString("F_PRODUCT_ATTR_UID"));						  
				picturesService.update(picture);
			} 
		}
	...
	public ModelAndView list(Page page){
		...
		ModelAndView mv = this.getModelAndView();
		try{
		 	mv.setViewName("business/tbproductattr/tb_product_attr_index"); 
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	...
	public Page dataList(){
		...
		try{ 
			page.setPd(pd);  
			varList = tbProductAttrService.list(page);	 
	...
	public ModelAndView goAdd(){ 
		...
		pd = this.getPageData();
		try {
			mv.setViewName("business/tbproductattr/tb_product_attr_add");
	...
	public PageData goEdit(){
		...
		pd = this.getPageData();
		try {
			pd = tbProductAttrService.findById(pd);	 
		} catch (Exception e) {
			logger.error(e.toString(), e);
		}						
		return pd;
	...
	public ModelAndView goEditView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/tbproductattr/tb_product_attr_edit"); 
	...
	public ModelAndView goDetailView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/tbproductattr/tb_product_attr_detail"); 
			mv.addObject("msg", "edit"); 
			mv.addObject("pd",pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
	...
	public Object deleteAll() { 
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				tbProductAttrService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
