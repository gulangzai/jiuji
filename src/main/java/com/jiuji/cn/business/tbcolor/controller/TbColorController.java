package com.jiuji.cn.business.tbcolor.controller; 
 ...
 */
@Controller
@RequestMapping(value="/tbcolor/tbColorController")
public class TbColorController extends BaseController {
	...
	public ResultAction save() throws Exception{
		...
		pd = this.getPageData();     
		int F_COLOR_UID = tbColorService.save(pd);
		...
		if(!"".equals(PICTURES_IDs)){
			for(int i=0;i<PICTURES_IDs.length;i++){
				...
				picture.put("TABLE_UID_VALUE",F_COLOR_UID);						  
	...
	public Page dataList(){
		...
		try{ 
			page.setPd(pd);  
			varList = tbColorService.list(page);	 
	...
	public Object deleteAll() { 
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				tbColorService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
