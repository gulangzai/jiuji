package com.jiuji.cn.business.tborder.controller; 
 ...
 */
@Controller
@RequestMapping(value="/tborder/tbOrderController")
public class TbOrderController extends BaseController {
	
	String menuUrl = "tborder/tbOrderController/list.do";  
	
	@Resource(name="tbOrderService")
	...
	public ResultAction save() throws Exception{
		logBefore(logger, "TbOrderController");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();     
		int F_ORDER_UID = tbOrderService.save(pd);
	...
	@ResponseBody
	public ResultAction operate(){
		logBefore(logger, "TbOrderController");
		ResultAction ra = new ResultAction(); 
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			tbOrderService.edit(pd); 
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		ra.setIserror(false);
	...
	public ModelAndView list(Page page){
		...
		ModelAndView mv = this.getModelAndView();
		try{
		 	mv.setViewName("business/tborder/tb_order_index"); 
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	...
	public String toMyOrder(Model model,HttpSession session,HttpServletRequest request){
		Page page =  new Page(); 
		PageData pd = this.getPageData();
		String userid = (String) session.getAttribute("F_USER_ID");
		pd.put("F_USER_ID", userid);
		page.setPd(pd);
		try {
			List<PageData>	varList = tbOrderService.list(page);
			model.addAttribute("tbOrders", varList);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}	
		return "/modules/person/myOrder";
	}
	
	@RequestMapping("/toWapMyOrder")
	public String toWapMyOrder(Model model,HttpSession session,HttpServletRequest request){
		Page page =  new Page(); 
		PageData pd = this.getPageData();
		String userid = (String) session.getAttribute("F_USER_ID");
		...
		return "/wapjiuji/order/my_order";
	}
	...
	public Page dataList(HttpSession session,Page page,String currentPage){
		...
		try{ 
			...
			pd.put("F_USER_ID", userid);
			page.setPd(pd);  
			varList = tbOrderService.list(page);
	 ...
	 * dataList
	 */
	@RequestMapping(value="/wapDataList")
	@ResponseBody
	public Page wapDataList(HttpSession session,HttpServletRequest request,Page page,String currentPage){
		logBefore(logger, "TbOrderController");   
		PageData pd = this.getPageData();
		List<PageData>	varList  = null;
		try{ 
	...
	public ModelAndView goAdd(){ 
		...
		pd = this.getPageData();
		try {
			mv.setViewName("business/tborder/tb_order_add");
	...
	public ModelAndView goEditView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/tborder/tb_order_edit"); 
	...
	public ModelAndView goDetailView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/tborder/tb_order_detail"); 
			mv.addObject("msg", "edit"); 
			mv.addObject("pd",pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
	...
	public Object deleteAll() { 
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				tbOrderService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
