package com.jiuji.cn.business.tbhot.controller; 
 ...
 */
@Controller
@RequestMapping(value="/tbhot/tbHotController")
public class TbHotController extends BaseController {
	
	String menuUrl = "tbcolor/tbColorController/list.do";  
	
	@Resource(name="tbHotService")
	private TbHotService tbHotService;
	...
	@ResponseBody
	public ResultAction save(TbHot tbHot) throws Exception{
		logBefore(logger, "TbColorController");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();     
		int F_COLOR_UID = tbHotService.save(tbHot); 
	...
	public ResultAction edit(HttpServletRequest request) throws Exception{
		...
		if(!"".equals(PICTURES_IDs)&&PICTURES_IDs!=null){
			for(int i=0;i<PICTURES_IDs.length;i++){
				...
				picture.put("TABLE_NAME", "TB_COLOR");			 
				picture.put("TABLE_UID_VALUE",pd.getString("F_COLOR_UID"));						  
	...
	public ModelAndView list(Page page){
		...
		ModelAndView mv = this.getModelAndView();
		try{
		 	mv.setViewName("business/tbcolor/tb_color_index"); 
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	...
	public Page dataList(){
		...
		try{ 
			page.setPd(pd);  
			varList = tbHotService.list(page);	 
	...
	public ModelAndView goAdd(){ 
		...
		pd = this.getPageData();
		try {
			mv.setViewName("business/tbcolor/tb_color_add");
	...
	public ModelAndView goEditView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/tbcolor/tb_color_edit"); 
	...
	public ModelAndView goDetailView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/tbcolor/tb_color_detail"); 
			mv.addObject("msg", "edit"); 
			mv.addObject("pd",pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
	...
	public Object deleteAll() { 
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				tbHotService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
