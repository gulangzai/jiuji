package com.jiuji.cn.business.tbmessage.controller; 
...
import com.jiuji.cn.business.tbmessage.service.TbMessageService;
 ...
 */
@Controller
@RequestMapping(value="/tbmessage/tbMessageController")
public class TbMessageController extends BaseController {
	
	String menuUrl = "tbmessage/tbMessageController/list.do";  
	...
	public ResultAction save() throws Exception{
		logBefore(logger, "TbMessageController");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();     
		int F_MESSAGE_UID = tbMessageService.save(pd);
		...
		if(!"".equals(PICTURES_IDs)){
			for(int i=0;i<PICTURES_IDs.length;i++){
				...
				picture.put("TABLE_NAME", "TB_MESSAGE");			 
				picture.put("TABLE_UID_VALUE",F_MESSAGE_UID);						  
	...
	public ResultAction edit(HttpServletRequest request) throws Exception{
		...
		if(!"".equals(PICTURES_IDs)&&PICTURES_IDs!=null){
			for(int i=0;i<PICTURES_IDs.length;i++){
				...
				PageData picture = picturesService.findById(pd); 
				picture.put("TABLE_NAME", "TB_MESSAGE");			 
				picture.put("TABLE_UID_VALUE",pd.getString("F_MESSAGE_UID"));						  
				picturesService.update(picture);
			} 
		}
	...
	public ModelAndView list(Page page){
		...
		ModelAndView mv = this.getModelAndView();
		try{
		 	mv.setViewName("business/tbmessage/tb_message_index"); 
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	...
	@ResponseBody
	public Page dataList(HttpSession session){
		logBefore(logger, "TbMessageController");  
		Page page =  new Page(); 
		...
		try{ 
			...
			page.setPd(pd); 
			varList = tbMessageService.list(page);	 
	...
	public ModelAndView goAdd(){ 
		...
		pd = this.getPageData();
		try {
			mv.setViewName("business/tbmessage/tb_message_add");
	...
	public PageData goEdit(){
		...
		pd = this.getPageData();
		try {
			pd = tbMessageService.findById(pd);	 
		} catch (Exception e) {
			logger.error(e.toString(), e);
		}						
		return pd;
	...
	public ModelAndView goEditView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/tbmessage/tb_message_edit"); 
	...
	public ModelAndView goDetailView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/tbmessage/tb_message_detail"); 
			mv.addObject("msg", "edit"); 
			mv.addObject("pd",pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
	...
	public Object deleteAll() { 
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				tbMessageService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
