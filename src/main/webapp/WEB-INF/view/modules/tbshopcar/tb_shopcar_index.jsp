...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid"> 
	<div class="col-sm-12" id="inside">
			...
			<form class="form-inline" width="100%" action="${ctx}/tbshopcar/tbShopCarController/list.do" method="post" name="INDEX_Form" id="INDEX_Form" style="border:solid 1px #ddd;line-height:50px;vertical-align:middle;">
		 	...
		 	<table id="jqGrid" class="table table-striped table-bordered table-hover imagetable"  action="${ctx}/tbshopcar/tbShopCarController/dataList.do">
	...
	<!-- PAGE CONTENT ENDS HERE -->
       ...
       <script type="text/javascript">
		 ...
		 function doReportInfo(cellvalue, opts, rowObject){
    	    var F_SHOPCARDID = rowObject.F_SHOPCARDID; 
    		
    		var showHtml =  "<button type='button' class='btn btn-default btn-sm' onclick='see("+F_SHOPCARDID+")'>";
    		showHtml +="<span class='glyphicon  glyphicon glyphicon-file'></span>";
    		showHtml +="</button>";
    		
    		showHtml +=  "<button type='button' class='btn btn-default btn-sm' onclick='edit("+F_SHOPCARDID+")'>";
    		showHtml +="<span class='glyphicon glyphicon-edit'></span>";
    		showHtml +="</button>";
		
    		showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='del("+F_SHOPCARDID+")'>";
		...
		function add(id){
		  ...
		  $.get("${ctx}/tbshopcar/tbShopCarController/goAddView.do?F_ORDER_UID="+id,function(data) {
		    	$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});
		...
		function edit(Id){
		    ...
		    $.get("${ctx}/tbshopcar/tbShopCarController/goEditView.do?F_ORDER_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
	   ...
	   function see(Id){
		    ...
		    $.get("${ctx}/tbshopcar/tbShopCarController/goDetailView.do?F_ORDER_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					...
					var url = "${ctx}/tbshopcar/tbShopCarController/delete.do?F_ORDER_UID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '${ctx}/tbshopcar/tbShopCarController/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='${ctx}/tbshopcar/tbShopCarController/excel.do';
		}
		</script>
