jQuery(function($){
	...
	var htmlShopCar ="";
	var shopCarListArray = shopCarList; 
	for (var i = 0, length = shopCarListArray.length; i < length; i++) {
		htmlShopCar+= '<li class="cart_item">';
		htmlShopCar+= "<div>";
		htmlShopCar+= '<div class="cart_item_pic"><a href="#"><img src="'+shopCarListArray[i].F_PATH+'"/></a></div>';
		htmlShopCar+= '<div class="cart_item_desc"><a href="#" class="cart_item_name">'+shopCarListArray[i].F_ProductName+'</a><div class="cart_item_sku"><span>尺码：38码（精工限量版）</span></div>';
		htmlShopCar+= '<div class="cart_item_price"><span class="cart_price">￥'+shopCarListArray[i].F_Price+'</span></div></div>';
		htmlShopCar+= "</div>";
		htmlShopCar+= '</li>'; 
	 } 
	console.info(htmlShopCar);
	//创建DOM
	var 
	...
	quickDataFns = {
		...
		message_list: {
			...
			content: '<div class="ibar_plugin_content">'
					    +' <div class="ibar_cart_group ibar_cart_product"><div class="ibar_cart_group_header"><span class="ibar_cart_group_title"></span><a href="#">我的购物车</a></div>'
					    +'<ul>'+htmlShopCar+'</ul>'
		              +'</div>'
		              ...
		              +'   </div><a href="#" class="cart_go_btn" target="_blank">去购物车结算</a>'
		              +'  </div>'
	               +'</div>',
			init:$.noop
		},
