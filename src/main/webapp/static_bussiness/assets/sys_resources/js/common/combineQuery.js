...
combineQuery.getQueryCombineData = function (queryform,orderform,tableid) {
	...
	if(tableid){
		var tId = "#"+tableid;
		if(jQuery(tId).attr("idisplaylength")){//每页行数
			pageNum=jQuery(tId).attr("idisplaylength");
			cpageNum=jQuery(tId).attr("pagenum");
		}else{
			cpageNum = jQuery("#"+tableid).jqGrid('getGridParam','page');
			pageNum = jQuery("#"+tableid).jqGrid('getGridParam','rowNum');
		}
	}
	if(false && tableid)
	{
	...
	if(tableid){
		var sortname = jQuery("#"+tableid).jqGrid('getGridParam','sortname');
		var order = jQuery("#"+tableid).jqGrid('getGridParam','sortorder');		
		if(sortname!=null&&sortname!=''){
			orderdata ="orders:[";
			orderdata +='{"order'+ '":"'+order+'"';
			orderdata +=',"fieldname'+ '":"'+sortname+'"}';
			orderdata += "]";	
		}
	}
//alert("orderdata===="+orderdata);	
	...
	//alert(orderdata);
	json = "{querycondition: {conditions: [" +querydata+" ]},pages:{recordsperpage:"+pageNum+", currentpagenum:"+cpageNum+", totalpage:0, countrows:0}";
